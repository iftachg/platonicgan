...
class EmissionAbsorptionRenderer():
    ...
    def render(self, volume, axis=2):
        ...
        bs = density.shape[0]
        sample_coords = self.sample_coords.expand(bs, self.sample_size_z, self.param.data.cube_len, self.param.data.cube_len, 3)  # Expand for batch size

        density = density * self.density_factor
        density = density / self.sample_size_z  # Normalize by depth?
